...
app.post("/api/run-stream", async (req, res) => {
  ...
  try {
    const final = await runPipeline({
      ...
      onAgentsGenerationStart: (info) => send("agents-generating-start", info),
      onAgentsGenerationDone: (info) => send("agents-generating-done", info),
  ...
  try {
    ...
    const record: RunRecord = {
      ...
      settings: {
        ...
        tailoredAgents: row.tailoredAgents,
      },
      ...
      report: row.report,
      generatedProfiles: row.generatedProfiles ? JSON.parse(row.generatedProfiles) : null,
